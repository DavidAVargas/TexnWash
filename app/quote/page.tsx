...
type Service = { emoji: string; label: string; value: string; price: string; subtitle?: string };
...
const RESIDENTIAL_SERVICES: Service[] = [
  ...
  {
    emoji: "✨", label: "Curb Appeal Package", value: "Curb Appeal Refresh Package",
    price: "Bundle — Save $50", subtitle: "Driveway + Walkway + Sidewalk",
  },
];
...
const TIME_SLOTS = [
  { label: "Morning", window: "8:00 AM – 12:00 PM", emoji: "🌅" },
  { label: "Afternoon", window: "12:00 PM – 4:00 PM", emoji: "☀️" },
  { label: "Evening", window: "4:00 PM – 8:00 PM", emoji: "🌆" },
];
...
function SectionLabel({ n, text }: { n: number; text: string }) {
  return (
    <div className="flex items-center gap-3 mb-5">
      <span className="w-7 h-7 rounded-full bg-[#BD5700] text-white text-xs font-bold flex items-center justify-center shrink-0">
        {n}
      </span>
...
export default function QuotePage() {
  ...
  const handleTermsScroll = () => {
    ...
    if (el && el.scrollTop + el.clientHeight >= el.scrollHeight - 16) {
      setTermsRead(true);
    }
  };
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (
        ...
        addressInputRef.current && e.target !== addressInputRef.current
      ) setShowSuggestions(false);
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedServices.length === 0) {
      Swal.fire({ icon: "warning", title: "Select a Service", text: "Please select at least one service.", confirmButtonColor: "#BD5700" }); return;
    }
    if (!preferredDate) {
      Swal.fire({ icon: "warning", title: "Pick a Date", text: "Please select a preferred service date.", confirmButtonColor: "#BD5700" }); return;
    }
    if (fullName.trim().split(/\s+/).length < 2 || /[^a-zA-Z\s'\-]/.test(fullName)) {
      Swal.fire({ icon: "warning", title: "Invalid Name", text: "Please enter your first and last name.", confirmButtonColor: "#BD5700" }); return;
    }
    if (phone.replace(/\D/g, "").length !== 10) {
      Swal.fire({ icon: "warning", title: "Invalid Phone", text: "Please enter a valid 10-digit phone number.", confirmButtonColor: "#BD5700" }); return;
    }
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
      Swal.fire({ icon: "warning", title: "Invalid Email", text: "Please enter a valid email address.", confirmButtonColor: "#BD5700" }); return;
    }
    if (address.length < 5) {
      Swal.fire({ icon: "warning", title: "Address Required", text: "Please enter your property address.", confirmButtonColor: "#BD5700" }); return;
    }
    if (!termsAccepted) {
      Swal.fire({ icon: "warning", title: "Terms Required", text: "Please read and accept the service terms.", confirmButtonColor: "#BD5700" }); return;
    }
    if (!isSignedIn && !recaptchaToken) {
      Swal.fire({ icon: "warning", title: "reCAPTCHA Required", text: "Please check the \"I'm not a robot\" box.", confirmButtonColor: "#BD5700" }); return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSelectedServices([]); setSelectedAddOns([]); setAddress(""); setSquareFootage("");
        setPreferredDate(undefined); setPreferredTime(""); setPhone(""); setNotes("");
        setTermsAccepted(false); setTermsRead(false); setRecaptchaToken("");
  ...
  return (
    <div className="bg-white">
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        {/* Clerk banner */}
        {isSignedIn && (
          <div className="flex items-center gap-3 bg-green-50 border border-green-100 rounded-2xl px-5 py-4 mb-10">
            <div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center shrink-0">
              <span className="text-green-600 text-sm font-bold">✓</span>
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-900">Signed in as {user.fullName}</p>
              <p className="text-xs text-gray-500">Your info is pre-filled · reCAPTCHA waived</p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          {/* ── 1. Services ── */}
          <div>
            <SectionLabel n={sectionNum++} text="What do you need cleaned?" />
                ...
                {availableAddOns.map((addon) => {
                  ...
                  return (
                    <label key={addon.label}
                      className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${checked ? "border-[#BD5700] bg-orange-50" : "border-gray-100 hover:border-gray-200 bg-white"}`}>
                      <input type="checkbox" className="accent-[#BD5700] w-4 h-4 shrink-0"
                        checked={checked} onChange={() => toggleAddOn(addon.label)} />
                      <span className="text-xl">{addon.emoji}</span>
                      <span className="flex-1 text-sm font-medium text-gray-700">{addon.label}</span>
                      <span className="text-sm font-bold text-[#BD5700] shrink-0">{addon.price}</span>
                    </label>
                  );
                })}
              </div>
              <div className="mt-4 flex items-start gap-2.5 bg-orange-50 rounded-xl px-4 py-3 text-xs text-gray-600">
                <span className="shrink-0 mt-0.5 text-[#BD5700]">ⓘ</span>
          ...
          {/* ── 3. Property Details ── */}
          <div className="border-t border-gray-100 pt-12 space-y-3">
            <SectionLabel n={sectionNum++} text="Property Details" />
          ...
          <div className="border-t border-gray-100 pt-12">
            ...
            {preferredDate && (
              <div className="flex items-center gap-2 bg-orange-50 rounded-xl px-4 py-3 text-sm text-gray-700 mb-6">
          ...
          <div className="border-t border-gray-100 pt-12 space-y-3">
            ...
            <input type="text" placeholder="Full Name" aria-label="Full Name"
              ...
              className={`${inputClass} ${isSignedIn ? "bg-gray-50 text-gray-500 cursor-default" : ""}`}
              readOnly={isSignedIn} required />
          ...
          {/* ── 6. Notes ── */}
          <div className="border-t border-gray-100 pt-12 space-y-3">
            <SectionLabel n={sectionNum++} text="Additional Notes" />
            ...
            <div className="flex items-start gap-2 text-xs text-gray-400">
              ...
              <span>
                ...
                <strong className="text-gray-600">(210) 201-2123</strong>{" "}
                after submitting — it helps us give you a more accurate quote.
              </span>
            </div>
          </div>
          ...
          {/* ── 7. Terms ── */}
          <div className="border-t border-gray-100 pt-12">
            <SectionLabel n={sectionNum++} text="Service Terms" />
              ...
              {showTerms && (
                <>
                  <div ref={termsScrollRef} onScroll={handleTermsScroll}
                    ...
                    <div>
                      <p className="font-semibold text-gray-800 mb-1">Property Damage</p>
                      <p>We take every precaution to protect your property and treat it with care. In the rare event that damage occurs as a direct result of our service, Tex N Wash will work with you to address it. Please note we are not responsible for damage to pre-existing conditions such as cracked paint, deteriorating surfaces, loose caulking, aging wood, or fragile fixtures that may be affected by cleaning. We will flag any concerns we notice before beginning work.</p>
                    </div>
                    <div>
            ...
            <label className={`flex items-start gap-3 mt-4 ${termsRead || !showTerms ? "cursor-pointer" : "cursor-not-allowed opacity-50"}`}>
              <input type="checkbox" checked={termsAccepted}
                onChange={(e) => termsRead && setTermsAccepted(e.target.checked)}
                disabled={!termsRead && showTerms}
